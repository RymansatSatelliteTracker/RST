...
- [x] （無線機接続環境がある場合）送信固定選択時：ダイヤルでRxを変更→Rxが追従し、ダイヤル操作終了後にTx表示・基準周波数がSumを保ったまま更新され、無線機のTx周波数も上書き送信されることを確認する（実機検証OK）
- [x] （無線機接続環境がある場合）受信固定選択時：上記と対称のケース（ダイヤルでRxを変更した場合の反映、ダイヤル操作終了後もRxが変化しないこと）を確認する（実機検証OK）
- [x] （無線機接続環境がある場合）衛星固定モードでは、従来どおりTx/Rxとも常時ドップラー追従することを確認する（リグレッション防止）（実機検証OK）
...
- [x] （無線機接続環境がある場合）送信固定・受信固定のいずれでも、Rxダイヤル操作により無線機側のTx周波数が自動的にズレても、RST側のTx表示・基準周波数が変化しないことを実機で確認する（送信固定側は実機検証OK。受信固定側でRx周波数自体がズレる別の不具合が発覚 → Phase 7で対応）

## Phase 7: 実機検証で発覚した不具合の修正（受信固定モードでダイヤル操作終了後にRx周波数がズレる）

- [x] `src/renderer/components/organisms/TransceiverCtrl/useTransceiverCtrl.ts` を修正
  - `resendFixedSideFreqToTransceiver()`からRx側の分岐（`!execRxDopplerShiftCorrection.value && isSatelliteMode.value`のブロック）を削除する
  - Tx側の分岐（`!execTxDopplerShiftCorrection.value`のブロック）は変更しない
  - 関数のJSDocコメントを、Tx側のみを対象とする内容に更新し、Rx側で再送信が不要な理由（ダイヤル操作時に画面表示へ直接反映済みであり、受信固定モードでは周期処理の対象外のため値が変化しないこと。現在時刻のドップラー式で再算出すると経過時間分ズレてしまうこと）を明記する
- [x] `npm run test` / `npm run ts` で全テスト成功・型エラーなしを確認する（`TransceiverRecvFreqResolver`側の変更はないため既存テストに変更は不要）
- [x] `doc/30_画面設計/G2_メイン.md` の「受信固定」の説明を、「ダイヤル操作終了後、直前のダイヤル操作で更新された基準周波数から改めてRx周波数を算出し、無線機へ送信して受信固定を継続する」という記載から、「ダイヤル操作で直接設定された値がそのまま維持される（再送信は不要）」という記載に修正する
- [x] （無線機接続環境がある場合）受信固定選択時：ダイヤルでRxを変更した後、3秒（`TRANSCEIVE_WAIT_MS`）待ってドップラーシフトが再開されても、Rx周波数がダイヤルした値のまま変化しないことを実機で確認する（実機検証OK）
