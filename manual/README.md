...
#### 2.1.4 方位・仰角表示（中央下）
メイン画面中央下の円形表示エリアには、選択した衛星の方位（Az） と 仰角（El） がリアルタイムで選択した衛星と同じ色の点で表示されます。
方位、仰角の数値はAutoがオンの場合に表示されます。
Autoボタン（ローテーター制御用） により以下の操作が可能です。
Autoがオンの場合
衛星の方位（Az）と仰角（El）の数値、またアンテナの向きが表示されます。
...
### 3.1.5 衛星個別追加
衛星をリストで選択していない状態で右上部の衛星アイコンをクリックすると衛星の個別登録を行うことができます。
### 📌 画面の構成
