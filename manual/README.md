...
選択された衛星の位置（各色の点）、軌道（線）、衛星の可視範囲（白い範囲）が表示されます
２つ以上設定した衛星は、青色、赤色などの各色の点で表示されます（最大10衛星）
...
#### 2.1.4 方位・仰角表示（中央下）
メイン画面中央下の円形表示エリアには、選択した衛星の方位（Az） と 仰角（El） がリアルタイムで表示されます。
方位（Az） は、アンテナを向けるべき水平方向を示します。
北（N）が0°、東（E）が90°、南（S）が180°、西（W）が270°として表示されます。
仰角（El） は、アンテナを向ける垂直方向（上方向）を示します。
水平を0°とし、真上（天頂方向）を90°として表示します。
また、このエリアには Autoボタン（ローテーター制御用） があり、以下の操作が可能です。
Autoがオンの場合
...
周波数の変更は、数値欄を直接クリックしてキーボードで入力するか、マウスのスクロールホイールで増減可能です。
受信周波数（Rx）
...
具体的には、アプリ設定（AppConfig）で事前に登録された周波数設定を使用します。
もしAppConfigに設定がない場合は、衛星のデフォルト設定（default_satellite）を検索し、その設定を使用することを通知するポップアップが表示され、自動的に設定が適用されます。
Autoがオフの場合
周波数を手動で自由に設定・調整できます。
...
ここでは、衛星通信で使用する変調方式を選択できます。
利用可能なモードは以下の通りです。

| **モード** | **説明** |
|-----------|---------|
| **USB（Upper Side Band）** | 上側波帯（SSB）を使用するモード（一般的な衛星通信用） |
| **LSB（Lower Side Band）** | 下側波帯（SSB）を使用するモード |
| **CW（Continuous Wave）** | モールス符号（CW通信）を使用するモード |
| **FM（Frequency Modulation）** | 周波数変調（FM通信）を使用するモード（アマチュア衛星でよく使用） |
...
| **モード** | **動作** |
|-----------|---------|
| **Normal（ノーマル）** | 通常の送信・受信周波数で通信 |
| **Reverse（リバース）** | 送信周波数と受信周波数を逆転（クロスバンド通信で使用） |


### **2.1.7 AOSリスト（右中）**
AOSリストは、今後の衛星通過（パス）に関する情報を一覧で表示するエリアです。
...
|------------|----------------------|
| **ヘルプ**   | 操作方法のヘルプを表示する      |
| **バージョン** | 現在のアプリのバージョンを表示 |


### 2.3 設定アイコン（画面左）
3.衛星アイコン（衛星設定）　4.ラジオアイコン（無線機設定）　5.アンテナアイコン（ローテーター設定）6.地球アイコン（地上局設定）
...
### 3.1.4 グループ管理
- **「Group1」プルダウンメニューを変更** して、別のグループに切り替え。
- **「グループ」ボタンで新しいグループを作成可能**

### 3.1.5 衛星個別追加
衛星をリストで選択していない状態で衛星アイコンをクリックすると衛星の個別登録を行うことができます。
### 📌 画面の構成
...
## 3.2. 衛星設定タブ2（TLE読み込み）
このタブでは、**TLE（Two-Line Element）を取得・更新** できます。
...
| IPアドレス    | LAN接続する場合のIPアドレス            |
| Port         | ネットワーク接続時の通信ポート          |
...
> ** Tx（送信周波数）と Rx（受信周波数）が連動している理由**


### ** サテライトモード（ノーマル / リバース）**
| **モード**   | **動作**                               |
|-----------|-----------------------------------|
| **ノーマル**  | 通常の送信・受信周波数で通信する           |
| **リバース**  | 送信と受信の周波数を逆転させる（特定の衛星で使用） |
...
## **6. DateTime（日時表示）**
画面右下の「Date Time」は、**現在の時間と衛星通過時間** を表示します。
